...
-- Steve Donovan, 2007
--
-- Edited to include the ObjLua, changed header, and changed table handling.
...
function compile(line)
    ...
    --[[local f,err = objlua.translate(line,'local')
    if err == nil then err = true end
    --if verbose and f then print(f) end
    
    if f then
		return true, loadstring(f, "local")
	end
    return err,f]]
    local f, err = objlua.loadstring(line, "<stdin>")
    return err,f
end

function evaluate(chunk)
    local vars = { pcall(chunk) }
    local ok = vars[1]
    if not ok then
        return vars[2]
    end
...
function eval_lua(line)
    ...
    if err then
        ...
        if line:sub(1, 1) == "=" then
            err,chunk=compile('_pretty_print(' .. line:sub(2) .. ')')
        end
    ...
    if not err then assert(type(chunk) == 'function') end
    if not err then
        err = evaluate(chunk)
    end
...
print "Interactive Lua prompt (ilua:ObjLua) "

package.path = "./?/init.lua;" .. package.path
--dofile"init.lua"
objlua = require'objlua'

print(objlua._FormatHeader())
